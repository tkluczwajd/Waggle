...
window.Waggle.toggleWikiAccordion = (el) => {
    try {
        ...
        console.error("Błąd akordeonu:", e);
    }
};
        ...
        if (tab === 'rasy') {
            ...
            if (item.filters && typeof item.filters === 'object') {
                ...
                for (const [key, value] of Object.entries(item.filters)) {
                    // 🔥 ZABEZPIECZENIE: Upewniamy się, że to na 100% liczba, a nie tekst
                ...
                <div style="margin-bottom: 12px; background: var(--panel-bg); border-radius: 20px; border: 1px solid var(--border-color); box-shadow: 0 4px 15px rgba(0,0,0,0.02); overflow: hidden;">
                    <div onclick="window.Waggle.toggleWikiAccordion(this)" 
                        ...
                        <div style="flex: 1; min-width: 0; display: flex; flex-direction: column; justify-content: center;">
                            <b style="font-size: 16px; color:var(--text-color); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; width: 100%; font-weight: 900; letter-spacing: -0.3px;">${item.title || "Brak nazwy"}</b>
            ...
            html += `
                <div style="margin-bottom: 12px; background: var(--panel-bg); border-radius: 20px; border: 1px solid var(--border-color); border-left: 5px solid var(--secondary); box-shadow: 0 4px 15px rgba(0,0,0,0.02); overflow: hidden;">
                    <div onclick="window.Waggle.toggleWikiAccordion(this)" 
                        ...
                        <b style="font-size: 15px; color:var(--text-color); font-weight: 800; flex: 1;">${item.title || "Porada"}</b>
...
export function openWikiDetails(id, tab) {} // Pusta wydmuszka by nie wywalać błędu w starych importach
