...
function renderWiki(tab) {
    let html = "";
    if (tab === 'rasy') WIKI.breeds.forEach(b => html += `<div class="post-card"><b>${b.name}</b><p style="margin:5px 0 10px;">${b.desc}</p><small style="color:var(--secondary);">Energia: ${b.energy}</small></div>`);
    else if (tab === 'trening') WIKI.training.forEach(t => html += `<div class="post-card" style="border-left:4px solid var(--secondary);"><b>${t.title}</b><p style="margin-top:5px;">${t.desc}</p></div>`);
    else if (tab === 'sytuacje') WIKI.situations.forEach(s => html += `<div class="post-card" style="border-left:4px solid var(--danger);"><b>${s.title}</b><p style="margin-top:5px;">${s.desc}</p></div>`);
    const container = document.getElementById('wiki-content'); if (container) container.innerHTML = html;
}
