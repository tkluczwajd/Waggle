...
export function renderCommentsList(comments) {
    let html = "";
    comments.forEach(c => {
        const timeStr = c.timestamp ? new Date(c.timestamp.toDate ? c.timestamp.toDate() : c.t// src/modules/community/postRenderer.js
import { appState as state } from '../../core/state.js';
...
export function renderPostsList(posts, filter) {
    ...
    // 🔥 POPRAWKA 1: Definiujemy currentUid, którego brakowało
    const currentUid = state.user?.uid || null; 
    
    posts.forEach(p => { 
        // 🔥 POPRAWKA 2: Tarcza ochronna. Jeśli jeden post jest uszkodzony, reszta i tak się załaduje!
        try {
            // Logika filtrowania - Sztywne zasady dla Info
            if (filter === 'events' && !p.isEvent) return;
            if (filter === 'alerts' && !p.isAlert) return;
            ...
            // Półprzezroczysty, szary krzyżyk usunięcia
            let delBtn = (p.uid === currentUid || isAdmin) ? `<button onclick="window.Waggle.deletePost('${p.id}')" style="position:absolute; top:12px; right:12px; background:rgba(0,0,0,0.08); border:none; color:var(--text-color); cursor:pointer; font-size:14px; width:28px; height:28px; border-radius:50%; display:flex; justify-content:center; align-items:center; z-index:10; transition: 0.2s;">✕</button>` : "";        
            
            // showUserActionModal zamiast openUserMenu 
            let userHeader = `<button onclick="window.Waggle.showUserActionModal('${p.uid}', '${p.author || 'Piesek'}', '${avatarSrc}')" style="background:none; border:none; padding:0; cursor:pointer; text-align:left; display:flex; align-items:center; gap:12px; width:100%;">
                <img src="${avatarSrc}" style="width:45px; height:45px; border-radius:50%; object-fit:cover; border:2px solid var(--border-color);">
                <div style="line-height:1.2;">
                    <b style="font-size:16px; color:var(--text-color);">${p.author || 'Piesek'}</b><br>
            ...
            // 🔥 POPRAWKA 3: Kuloodporne sprawdzanie i liczenie lajków!
            let likesCount = 0;
            if (typeof p.likes === 'number') {
                likesCount = p.likes;
            } else if (Array.isArray(p.likes)) {
                likesCount = p.likes.length;
            } else if (Array.isArray(p.likedBy)) {
                likesCount = p.likedBy.length;
            }

            const hasLiked = currentUid && Array.isArray(p.likedBy) && p.likedBy.includes(currentUid);
            const commentCount = p.commentCount || 0; 
            ...
            console.warn("Pominięto uszkodzony post: ", p.id);
        }
    }); 
    
    if(!html) html = `<div style="text-align:center; padding:40px 20px; color:var(--text-muted);"><h3>Brak wpisów 🐕</h3><p>Bądź pierwszy i dodaj posta!</p></div>`; 
    
    // 🔥 POPRAWKA 4: Prawidłowe ID kontenera z Twojego pliku index.html
    const container = document.getElementById('board-feed-container') || document.getElementById('posts-container');
    if(container) container.innerHTML = html; 
}
...
};imestamp).toLocaleTimeString('pl-PL', {hour: '2-digit', minute:'2-digit'}) : '';
        html += `
            <div style="background:var(--panel-bg); padding:10px 15px; border-radius:12px; margin-bottom:10px; text-align: left;">
                <div style="display:flex; justify-content:space-between; align-items:baseline;">
                    <b style="font-size:14px; color:var(--text-color);">${c.author}</b>
