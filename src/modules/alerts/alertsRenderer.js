...
// 🔥 Ustawiamy czas życia alertu na 3 godziny (w milisekundach)
const ALERT_LIFESPAN_MS = 3 * 60 * 60 * 1000; 

export function renderAlerts(alerts) {
    const L = window.L;
    const now = Date.now();
    let activeAlertsCount = 0; // Licznik faktycznie aktywnych alertów

    // Czyszczenie mapy przed nałożeniem nowych/przefiltrowanych alertów
    mapManager.clearLayer('alerts');
    alertMarkers = {}; // Resetujemy referencje

    let html = ""; // HTML dla bocznego panelu (jeśli istnieje)

    // Pętla przez wszystkie alerty pobrane z bazy
    alerts.forEach(a => {
        // Konwertujemy timestamp alertu na wartość liczbową
        const alertTime = a.createdAt.toDate ? a.createdAt.toDate().getTime() : new Date(a.createdAt).getTime();
        
        // 🚨 KRYTYCZNY TEST: Sprawdzamy, czy alert jest "świeży" (< 3h)
        if (now - alertTime < ALERT_LIFESPAN_MS) {
            
            // Jeśli użytkownik go nie odrzucił (nie kliknął krzyżyka)
            if (!dismissedAlerts.includes(a.id)) {
                activeAlertsCount++;
                
                // Rysujemy na mapie tylko te aktywne
                ...
                // Budujemy też HTML dla panelu (jeśli jest potrzebny)
                html += `<div class="post-card" style="margin-bottom:10px; border-left:4px solid var(--danger);">
                            <div style="display:flex; justify-content:space-between; align-items:start;">
                                <div style="flex:1;">
                                    ...
                                    <small style="color:var(--text-muted);">Zgłoszono: ${new Date(alertTime).toLocaleTimeString()}</small>
                                </div>
    ...
    // Pigułka nad mapą pokazuje się tylko, gdy jest co najmniej 1 ŚWIEŻY alert
    const alertPill = document.getElementById('active-alert-pill');
    if (alertPill) {
        alertPill.style.display = activeAlertsCount > 0 ? 'flex' : 'none';
    }
