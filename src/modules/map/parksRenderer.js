...
export function renderParksOnMap(places) {
    // Używamy globalnego obiektu Leaflet
    const L = window.L;
    ...
    places.forEach(place => {
        const iconHtml = `<div style="background:${place.isDogPark ? '#4cd137' : '#00a8ff'}; width:30px; height:30px; border-radius:50%; display:flex; align-items:center; justify-content:center; color:white; border:2px solid white; box-shadow:var(--soft-shadow); font-size:16px;">${place.isDogPark ? '🐕' : '🌳'}</div>`;
