...
function updateAlertHubUI() {
    ...
    if (nearby.length > 0) {
        ...
        pill.onclick = () => showAllAlertsPopup(nearby); // Wywoła nowy PIĘKNY HTML Modal
...
// 🌟 PIĘKNY MODAL DLA ALERTÓW (Zastępuje alert())
window.showAllAlertsPopup = function(nearbyAlerts) {
    ...
    nearbyAlerts.forEach(a => {
        ...
        html += `<div style="padding: 15px; border-radius: 16px; background: var(--bg-color); margin-bottom: 10px; border-left: 4px solid var(--danger);">
                    ...
                    <small style="color: var(--text-muted); font-weight: 800;">Około ${dist} km stąd</small>
                 </div>`;
...
async function loadParks(lat, lng) {
    ...
    try {
        ...
        data.elements.forEach(el => {
            ...
            // 🌟 PRZYWRÓCONA NAWIGACJA DO DRZEWEK
    ...
    } catch(err) { console.error("Overpass error:", err); parksLoaded = false; }
}
    ...
    const unsub = db.collection("walks").onSnapshot(snap => {
        ...
        snap.forEach(doc => {
            ...
            if(d.uid !== state.user.uid) {
                ...
                html += `<div class="walk-card" onclick="Waggle.centerOnTarget(${d.lat}, ${d.lng})"><img src="${d.avatar}"><div style="font-size:12px; font-weight:900; margin-top:5px;">${d.name}</div></div>`;
                if (dogMarkers[d.uid]) dogMarkers[d.uid].setLatLng([d.lat, d.lng]);
                else dogMarkers[d.uid] = L.marker([d.lat, d.lng], { icon: L.divIcon({ className: '', html: `<div style="width:40px;height:40px;border-radius:50%;border:3px solid white;overflow:hidden;background:white;box-shadow:var(--soft-shadow); box-sizing: border-box;"><img src="${d.avatar}" style="width:100%;height:100%;object-fit:cover;"></div>`, iconSize: [40, 40] }) }).addTo(state.map);
            }
        });
